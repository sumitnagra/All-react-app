...
<p>Introducing the Task NoteBook app, this app having the feature of login ,signup and logout with the backend support of Node and Mongodb . This app can create multiple account which are he secured by the security of JWT . Task can be added, deleted and can be modified according to the user The backend, developed in Node.js, provides a robust and scalable foundation for the application. It facilitates seamless communication between the client-side and the MongoDB database, ensuring efficient storage and retrieval of user-related information. 
    </p><button className='btn btn-dark' onClick={()=>{navigate("/notebook");window.scrollTo(0,110)}}>Try It</button></div><img src={Project7}/>
</div>
<div className='firstProject ' data-aos="fade-up">
    <div className='textsection' ><h2>07 Todo List </h2>
<p> This small application help user to create a todo list of the day ,the user can add task ,delete task and capable in modify the existed task  </p><button className='btn btn-dark' onClick={()=>{navigate("/app");window.scrollTo(0,110)}}>Try It</button></div><img src={Project6} />
</div>
<div className='firstProject ' data-aos="fade-up">
    <div className='textsection' ><h2>08 Shoping Cart </h2>
<p> The smart add to cart application in which you can choose different product from the list and buy whenever you want also you can remove the product from the list , great simple user interface . See the full details of the product after add to cart   </p><button className='btn btn-dark' onClick={()=>{navigate("/shop");window.scrollTo(0,110)}}>Try It</button></div><img src={Project8}/>
</div>
<Footer/>
    </>)
}
